# Brilliantly stolen from BackgrounDRb (minus the parse_part, that is my refactor...)
...
module Minions
  class CronTrigger
    ...
    # =============================================================================
    def day_restricted?; !@day.eql?(1..31); end
    def wday_restricted?; !@wday.eql?(0..6); end
    ...
    def day=(day);     @day = parse_part(day, 1 .. 31);     end
    def month=(month); @month = parse_part(month, 1 .. 12); end
    def year=(year);   @year = parse_part(year);            end
    def wday=(wday);   @wday = parse_part(wday, 0 .. 6);    end
    # =============================================================================
    ...
    private
      ...
      # =============================================================================
      # 0-5,8,10; 0-5; *; */5
      def parse_part expression, range=nil
        return range if expression.nil? or expression == '*' or expression =~ /^[*0]\/1$/

        expression.split(',').reduce([]) do |acc, part|
          acc + if part =~ /(\d+)-(\d+)/  # 0-5
                  ($1.to_i .. $2.to_i).to_a
                elsif part =~ /(\*|\d+)\/(\d+)/ && range  # */5, 2/10
                  ($1.to_i .. range.end).step($2.to_i).to_a
                else # 2
                  [part.to_i]
                end
        end
